...
export class CurriculumLoader {
    ...
    loadTutorInstructions() {
        if (this.tutorInstructionsCache)
    ...
    findByGradeAndSubject(grade, subject) {
        ...
        return this.loadSubjects().find((item) => normalize(item.grade) === targetGrade && normalize(item.subject) === targetSubject);
    }
    findSubject(subject, grade) {
        const targetSubject = normalize(subject);
        return this.loadSubjects().find((item) => {
            if (grade && normalize(item.grade) !== normalize(grade))
                return false;
    ...
    search(query, options) {
        ...
        return this.loadSubjects()
            .filter((item) => {
            if (options?.grade && normalize(item.grade) !== normalize(options.grade))
                return false;
            if (options?.subject && normalize(item.subject) !== normalize(options.subject))
                return false;
