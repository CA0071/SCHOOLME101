...
test("list_all_servers returns 42 servers", async () => {
  ...
  const payload = (result.structuredContent as { result: { count: number } }).result;
...
test("get_ai_tutor_instructions returns content", async () => {
  ...
  const payload = (result.structuredContent as { result: { content: string } }).result;
...
test("proxy tools are registered and routable", async () => {
  ...
  const payload = (result.structuredContent as { result: { proxied: boolean; server: string } }).result;
